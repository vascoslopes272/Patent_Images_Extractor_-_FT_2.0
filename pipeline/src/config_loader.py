...
def _find_config() -> Path:
    """
    Walk up from this file's location until config.yaml is found.
    ...
    for candidate in [here, here.parent, here.parent.parent]:
        ...
        if p.exists():
            return p
    raise FileNotFoundError("config.yaml not found starting from " + str(here))
...
def load_config(config_path: str | Path | None = None) -> dict:
    """
    Load config.yaml and return everything as a plain Python dict.

    Steps performed:
      1. Find and parse config.yaml with yaml.safe_load
      2. Convert every value in cfg["paths"] from a string → absolute Path
         (relative paths like "data/pdfs" become /home/vasco/data/patents/pdfs)
      3. Resolve cfg["extractor"]["model_path"] the same way
         (so notebooks in pipeline/notebooks/ find the model in pipeline/models/)
      4. Read .env and inject DRIVE_PATH as cfg["paths"]["drive_root"]

    Parameters
    ----------
    config_path : optional — pass an explicit path to config.yaml.
    ...
    # ── Step 1: locate and parse config.yaml ──────────────────────────────
